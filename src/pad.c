...
inline size_t getNumBlocks(const size_t input_length, const SkeinSize_t state_size)
{
    uint64_t blocks = 0;
    size_t bytes = 0;
    while(bytes < input_length)
    {
      bytes += (size_t) state_size/8;
      blocks++;
    }
...
inline size_t getPadSize(const uint64_t input_length, const SkeinSize_t state_size)
{
