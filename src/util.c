...
static char hexNibbleLookup(uint8_t hex_nibble)
{
...
uint_fast8_t bin2GrayByte(uint_fast8_t bin) { return (bin ^ (bin >> 1)); }

uint64_t bin2Gray(uint64_t bin) { return (bin ^ (bin >> 1)); }
